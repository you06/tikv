...
use tikv::server::server::GRPC_THREAD_PREFIX;
...
fn run_raft_server(pd_client: RpcClient, cfg: &TiKvConfig, security_mgr: Arc<SecurityManager>) {
    ...
    let env = Arc::new(
        EnvBuilder::new()
            ...
            .name_prefix(GRPC_THREAD_PREFIX)
            .build(),
    );
    ...
    let builder = ServerBuilder::new(env.clone()).register_service(create_backup(backup_service));
    ...
    backup_worker
        ...
        .unwrap()
        .join()
        .unwrap_or_else(|e| fatal!("failed to stop backup endpoint: {:?}", e));
