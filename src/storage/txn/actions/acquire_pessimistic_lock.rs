...
use crate::storage::{
    mvcc::{
        metrics::{
            ...
            MVCC_PESSIMISTIC_LOCK_ALLOW_CONFLICT_COUNTER,
            MVCC_PESSIMISTIC_LOCK_ALLOW_CONFLICT_GAP_HISTOGRAM,
        },
...
pub fn acquire_pessimistic_lock<S: Snapshot>(
    ...
    if let Some(lock) = exist_lock.as_mut() {
        ...
        } else if lock.contains_start_ts(reader.start_ts) {
            let requested_for_update_ts = for_update_ts;
            let existing_sub_lock = lock
                .find_shared_lock_txn(reader.start_ts)
                .expect("shared lock should contain sub lock");
            let locked_with_conflict_ts =
    ...
    if let Some((commit_ts, write)) = reader.seek_write(&key, TimeStamp::max())? {
        ...
        if commit_ts > for_update_ts {
            ...
            if allow_lock_with_conflict {
                MVCC_PESSIMISTIC_LOCK_ALLOW_CONFLICT_COUNTER.inc();
                let ts_gap = commit_ts
                    .into_inner()
                    .saturating_sub(for_update_ts.into_inner()) as f64;
                MVCC_PESSIMISTIC_LOCK_ALLOW_CONFLICT_GAP_HISTOGRAM.observe(ts_gap);
    ...
    if is_shared {
        txn.put_shared_pessimistic_lock(key, exist_lock, lock);
...
pub mod tests {
    ...
    fn test_acquire_shared_lock_creates_shared_entry() {
        ...
        let shared_lock = load_lock(&mut engine, key);
